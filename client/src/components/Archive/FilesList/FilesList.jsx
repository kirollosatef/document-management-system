...
import { setPosition, setSelectedItem } from "@store/toolsbar/toolsbarSlice";
...
function FilesList() {
  ...
  const api = "http://192.168.88.96:8080";
  const handleClick = (obj) => {
    dispatch(setSelectedItem({ type: "file", item: obj }));
  };
  useEffect(() => {
    if (archive?.files?.length >= 8) {
      ...
      {archive?.files?.length > 0 ? (
        <Grid sx={{ flexGrow: 1 }} container spacing={2}>
          {archive?.files?.map((item, i) => {
            ...
            const imgUrl = `/uploads/${item.path}`;
            return (
              <Grid key={item._id} item sm={3} xs={6} onClick={() => handleClick(item)}>
                <div className={`files-item  ${selectedItem.item._id === item._id ? "active" : ""}`}>
                  ...
                  <img
                    ...
                    width={"100%"}
                    height={"200px"}
                    style={{ padding: isPdf ? "2rem" : "1rem", objectFit: "contain" }}
